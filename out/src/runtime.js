...
class LibSdbRuntime extends events_1.EventEmitter {
            ...
            if (data.content.specialEvent === "jump") {
                ...
                if (this._priorStepData && this._priorStepData.source) {
                    if (this._priorStepData.source.jump === "i") {
                        ...
                        const nodePrior = utils_1.LibSdbUtils.SourceMappingDecoder.findNodeAtSourceLocation("FunctionDefinition", this._priorStepData.source, { AST: contract.ast });
                        ...
                        const node = utils_1.LibSdbUtils.SourceMappingDecoder.findNodeAtSourceLocation("FunctionDefinition", sourceLocation, { AST: contract.ast });
                            ...
                            if (variable.location === types_1.LibSdbTypes.VariableLocation.Storage) {
                                if (variable.position === null) {
                                    ...
                                    if (variable.refType === types_1.LibSdbTypes.VariableRefType.None) {
                                        ...
                                        value = utils_1.LibSdbUtils.interperetValue(variable.type, new bn_js_1.BN(content.value));
                                    }
                                    else {
                                        if (variable.refType === types_1.LibSdbTypes.VariableRefType.Array && !variable.arrayIsDynamic) {
                                            ...
                                            for (let j = 0; j < variable.arrayLength; j++) {
                                                ...
                                                const content = yield this._interface.requestStorage(address, key);
                                                values.push(utils_1.LibSdbUtils.interperetValue(variable.type, new bn_js_1.BN(content.value)));
                                            }
    ...
    sendVariableDeclarations(address) {
        return __awaiter(this, void 0, void 0, function* () {
            const contract = this._contractsByAddress.get(address);
            const declarations = [];
            if (contract) {
                ...
                astWalker.walk(contract.ast, (node) => {
                    if (node.name === "VariableDeclaration" && node.src) {
                        ...
                        const index = utils_1.LibSdbUtils.SourceMappingDecoder.toIndex(sourceLocation, contract.srcmapRuntime);
                        if (index !== null) {
                ...
                if (declarations.length > 0) {
                    yield this._interface.requestSendVariableDeclarations(contract.address, declarations);
                }
            }
        });
    }
