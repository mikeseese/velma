...
(function (LibSdbCompile) {
    ...
    function linkCompilerOutput(_files, _contractsByName, _contractsByAddress, sourceRootPath, compilationResult) {
        ...
        for (let i = 0; i < fileKeys.length; i++) {
            const absoluteSourcePath = path_1.join(sourceRootPath, fileKeys[i]);
            ...
            for (let j = 0; j < contractKeys.length; j++) {
                ...
                if (absoluteSourcePath !== null) {
                    if (!_files.has(absoluteSourcePath)) {
                        _files.set(absoluteSourcePath, new types_1.LibSdbTypes.File(sourceRootPath, relativeSourcePath));
                    }
                    ...
                    file.relativeDirectory = path_1.dirname(relativeSourcePath);
                    if (!file.sourceCode) {
        ...
        for (let i = 0; i < keys.length; i++) {
            const source = keys[i];
            const sourcePath = path_1.join(sourceRootPath, source);
